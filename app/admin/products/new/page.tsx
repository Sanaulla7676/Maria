...
type Variant = { size: Size; price: number; stock: number; active: boolean }

export default function NewProductPage() {
  const [variants, setVariants] = useState<Variant[]>(sizes.map((size) => ({ size, price: size === 30 ? 600 : size === 50 ? 1000 : 1800, stock: 0, active: true })))
  const addVariant = () => setVariants((v) => [...v, { size: 30, price: 0, stock: 0, active: true }])
  return <main className="container admin-page">
    ...
    <section className="detail-card variants-editor"><div className="section-head"><div><span className="kicker">Variants</span><h2>Size, price & stock</h2></div><button className="button" onClick={addVariant}><Plus size={15}/> Add variant</button></div>
      {variants.map((variant, index) => <div className="variant-row" key={index}><select value={variant.size} onChange={(e) => setVariants((v) => v.map((x,i)=>i===index?{...x,size:Number(e.target.value) as Size}:x))}>{sizes.map((size)=><option key={size} value={size}>{size}ml</option>)}</select><label>Price<input type="number" min="0" value={variant.price} onChange={(e)=>setVariants((v)=>v.map((x,i)=>i===index?{...x,price:Number(e.target.value)}:x))}/></label><label>Stock<input type="number" min="0" value={variant.stock} onChange={(e)=>setVariants((v)=>v.map((x,i)=>i===index?{...x,stock:Number(e.target.value)}:x))}/></label><label className="toggle"><input type="checkbox" checked={variant.active} onChange={(e)=>setVariants((v)=>v.map((x,i)=>i===index?{...x,active:e.target.checked}:x))}/> Active</label><button className="icon-button" title="Remove variant" onClick={()=>setVariants((v)=>v.filter((_,i)=>i!==index))}><Trash2 size={17}/></button></div>)}
    </section>
  </main>
}
