...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const productIndex = catalog.indexOf(productName)
  const productId = `maria-demo-product-${productIndex + 1}`
  const variantIds = { 30: `maria-demo-variant-${productIndex + 1}-30`, 50: `maria-demo-variant-${productIndex + 1}-50`, 100: `maria-demo-variant-${productIndex + 1}-100` } as const
  return <ProductDetail name={productName} productId={productId} variantIds={variantIds} />
}

function ProductDetail({ name, productId, variantIds }: { name: string; productId: string; variantIds: Record<30 | 50 | 100, string> }) {
  ...
  return <main className="container product-detail-page">
    ...
    <section className="product-detail">
      ...
      <div className="product-detail-copy">
        ...
        <div className="quantity-row"><span>Quantity</span><div className="quantity-control"><button onClick={() => setQuantity(Math.max(1, quantity - 1))}><Minus size={15}/></button><strong>{quantity}</strong><button onClick={() => setQuantity(Math.min(20, quantity + 1))}><Plus size={15}/></button></div></div>
        <div className="product-actions"><ProductCartAction productId={productId} variantId={variantIds[size]} quantity={quantity} price={price}/><button className={saved ? 'icon-button active' : 'icon-button'} aria-label="Save fragrance" onClick={() => setSaved(!saved)}><Heart fill={saved ? 'currentColor' : 'none'} size={19}/></button></div>
      </div>
    </section>
  </main>
}
